...
// 매크로의 경우 자료형이 존재하지 않으므로 메모리 공간을 가지고 잇지 않습니다.
...
#pragma region 전처리기
// 프로그램이 컴파일되기 이전에 프로그램에 대한 사전 처리를 하는 과정입니다.

// 전처리기는 컴파일러가 아닌 선행처리기에 의해서 처리되는 문자이기 때문에
// 명령문 끝에 ";"을 사용하지 않습니다.
// printf("num1234 변수의 값 : %d\n", num1234);
